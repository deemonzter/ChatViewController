    ...
    if (self.autoHideRightButton && !self.isEditing)
    {
        ...
        if (self.window) {
            ...
            [UIView animateWithDuration:0.5 animations:^{
                if ([self slk_appropriateRightButtonWidth] > 0) {
            ...
            [self slk_animateLayoutIfNeededWithBounce:bounces options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionAllowUserInteraction animations:NULL completion:^(BOOL finished) {
                // do nothing
            }];
        }
        else {
