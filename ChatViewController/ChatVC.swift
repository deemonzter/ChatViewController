...
class ChatVC: SLKTextViewController {
    ...
    func commonInit() {
        ...
        NotificationCenter.default.addObserver(self,  selector: #selector(ChatVC.textInputbarDidMove(_:)), name: NSNotification.Name.SLKTextInputbarDidMove, object: nil)
    }
    ...
    override func viewDidLoad() {
        ...
        self.rightButton.setImage(UIImage(named: "send"), for: UIControlState())
        self.rightButton.imageEdgeInsets = UIEdgeInsets(top: 4, left: 4, bottom: 4, right: 4)
        ...
        self.tableView.register(UINib(nibName: "GSChatCell", bundle: nil), forCellReuseIdentifier: GSChatCell.cellIdentifier())
        ...
        self.textView.placeholder = "Reply";
...
extension ChatVC {
    ...
    func configureDataSource() {
        ...
        for _ in 0..<10 {
            ...
            if message.username.characters.count % 2 == 0 {
                message.isUser = true
            } else {
    ...
    func configureActionItems() {
        ...
        let arrowItem = UIBarButtonItem(image: UIImage(named: "icn_arrow_down"), style: .plain, target: self, action: #selector(ChatVC.hideOrShowTextInputbar(_:)))
        let editItem = UIBarButtonItem(image: UIImage(named: "icn_editing"), style: .plain, target: self, action: #selector(ChatVC.editRandomMessage(_:)))
        let typeItem = UIBarButtonItem(image: UIImage(named: "icn_typing"), style: .plain, target: self, action: #selector(ChatVC.simulateUserTyping(_:)))
        let appendItem = UIBarButtonItem(image: UIImage(named: "icn_append"), style: .plain, target: self, action: #selector(ChatVC.fillWithText(_:)))
        let pipItem = UIBarButtonItem(image: UIImage(named: "icn_pic"), style: .plain, target: self, action: #selector(ChatVC.togglePIPWindow(_:)))
...
extension ChatVC {
    ...
    func messageCellForRowAtIndexPath(_ indexPath: IndexPath) -> GSChatCell {
        
        let cell = self.tableView.dequeueReusableCell(withIdentifier: GSChatCell.cellIdentifier()) as! GSChatCell
        
        if cell.gestureRecognizers?.count == nil {
            let longPress = UILongPressGestureRecognizer(target: self, action: #selector(ChatVC.didLongPressCell(_:)))
        ...
        cell.labelDateAndStatus.text = "Empty Date"
        cell.labelDateAndStatus.textColor = UIColor.lightGray
        
        if message.isUser {
            cell.viewImageBackground.image = UIImage(named: "chatbubble_green")
            cell.viewText.textColor = UIColor.white
            cell.viewText.textAlignment = .right
            cell.labelDateAndStatus.textAlignment = .right
            cell.constraintContainerLeading.constant = self.view.bounds.width * 0.2
            cell.constraintContainerTrailing.constant = 0
        } else {
            cell.viewImageBackground.image = UIImage(named: "chatbubble_white")
            cell.viewText.textColor = UIColor.darkGray
            cell.viewText.textAlignment = .left
            cell.labelDateAndStatus.textAlignment = .left
            cell.constraintContainerLeading.constant = 0
            cell.constraintContainerTrailing.constant = self.view.bounds.width * 0.2
        }
        
        cell.viewText.isEditable = false
        cell.viewText.backgroundColor = UIColor.clear
        cell.viewText.text = message.text
        cell.viewText.isScrollEnabled = false
